...
model = Sequential()
model.add(Bidirectional(LSTM(100, return_sequences=True), input_shape=(4,1)))  # => return_sequences=True LSTM안에 넣어줘야 함.
#model.add(LSTM(64))
model.add(GRU(64))
# => GRU, LSTM 뭐냐는 중요하지 않음 이빨이 맞냐 안 맞냐가 중요함
